...
#ifndef CONTROLNETPRIORITIESCLIENT_H
#define CONTROLNETPRIORITIESCLIENT_H
#include <omnetpp.h>
#include <fstream>
#include "Util.h"

/**
 * This is the simulation client for the Priorities case in ControlNet. Priority messages are
 * transmitted in the scheduled area, non-priority messages in the unscheduled area. Only node
 * 1 or one another noder are allowed to send priority messages. Node 1 sends priority and
 * non-priority, the ohter node only priority messages.
 */
class ControlNetPrioritiesClient : public cSimpleModule
{
private:
	/**
...
public:
	 ...
	 */
	Module_Class_Members(ControlNetPrioritiesClient, cSimpleModule, 0);
 ...
 * See OMNeT's user manual.</i>
 */
Define_Module(ControlNetPrioritiesClient);
